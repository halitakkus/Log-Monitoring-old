...
namespace Application.Packages.JWT.Filters
{
    ...
    public class ApplicationTokenAuthFilter : ActionFilterAttribute
    {
        public ITokenService TokenService => _tokenService;

        private readonly ITokenService _tokenService;

        public ApplicationTokenAuthFilter(ITokenService tokenService)
        {
        ...
        {
            // Ignore validation when action contains IgnoreTokenAuthFilter
            if (CheckMethodContainsIgnoreTokenAuthFilter(context))
            {
                return;
            }
        ...
        /// <summary>
        /// Check action has IgnoreTokenAuthFilter.
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        private bool CheckMethodContainsIgnoreTokenAuthFilter(ActionExecutingContext context)
        {
            return context.Filters.Any(filter => filter.GetType().Name == nameof(ApplicationIgnoreTokenAuthFilter));
        }
